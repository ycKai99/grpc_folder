...
class LoggingService {
    ...
    async subscribe(MessageLog_Incoming, AppLocation, LogLocation, AppLogLoc, AppProfile, localState, element) {
        ...
        const MessageLog = MessageLog_Incoming.pipe(rxjs_1.map(incoming_message => {
            let finalResponse;
            // console.log('incoming_message: ', incoming_message)
            ...
            if ("header" in incoming_message) {
                ...
                let message = incoming_message; // Type assertion, reinforce, telling the code this is strictly MessageLog
    ...
    async findFromMongo(args) {
        ...
        if (key == 'msgPayload') {
            ...
            let query = {}; // Trump card -- Search with any key
