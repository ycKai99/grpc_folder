...
exports.new_leave_application_notification_class = void 0;
...
class new_leave_application_notification_class extends base_notification_handler_1.base_notification_handler_class {
    constructor() {
        super();
                    ...
                    let newSub = DP_LeaveNotification.send(appname, msg).subscribe({
                        next: (response_msg) => {
                            ...
                            if (response_msg.data &&
                                ...
                                console.log("Current UCP id in new: " + currentUCPId);
                            }
                            if (currentUCPId) {
    ...
    subscribeForCheckLeaveNewApplication(inputObservable) {
        let newOutputObservable = new rxjs_1.Subject();
        inputObservable.subscribe({
            next: (process) => {
                if (process.eventType == handlers_manager_1.eventTypes.CheckLeaveCreation) {
                    ...
                    if (operation_data > "" && operation_data.toLowerCase() == "new") {
                        console.log("New Leave Detected..");
    ...
    subscribeForRetrieveNewLeave(inputObservable) {
        let newOutputObservable = new rxjs_1.Subject();
        inputObservable.subscribe({
            next: (process) => {
                if (process.eventType == handlers_manager_1.eventTypes.RetrieveLeave) {
                    ...
                    console.log("Retrieving New Leave and other details...");
                                                        ...
                                                        this.send_message(process.requestId, new_step_ind_5).then((response) => {
                                                            ...
                                                            if (status == 1) {
                                                                ...
                                                                this.send_message(process.requestId, new_step_ind_6).then((response) => {
                                                                    ...
                                                                    // console.log("LEAVE DETAILS : ",new_message["additionalLeaveDetails"]['ps_doc_header_ps_doc_remarks'])
    ...
    subscribeForUpdateNewApplicationCalenderLeave(inputObservable) {
        let newOutputObservable = new rxjs_1.Subject();
        inputObservable.subscribe({
            next: (process) => {
    ...
    subscribeForCheckNewCalenderLeave(inputObservable) {
        let newOutputObservable = new rxjs_1.Subject();
        inputObservable.subscribe({
            next: (process) => {
...
exports.new_leave_application_notification_class = new_leave_application_notification_class;
//# sourceMappingURL=new_leave_application_notification_handler.js.map