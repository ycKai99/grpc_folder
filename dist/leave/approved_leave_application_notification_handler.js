...
exports.approved_leave_application_notification_class = void 0;
...
class approved_leave_application_notification_class extends base_notification_handler_1.base_notification_handler_class {
    constructor() {
        super();
        ...
        let newSub = inputObservable.subscribe({
            next: (process) => {
                if (process.eventType == handlers_manager_1.eventTypes.Start
                    ...
                    //console.log(">> Approbation eventype start started")
                    ...
                    let newSub = DP_LeaveNotification.send(appname, msg).subscribe({
                        next: (response_msg) => {
                            if (response_msg.data &&
                                ...
                                //console.log("Approbation Notification Login Exception Error!")
                                this.errorSinkObservable.next(connectionError);
                            }
                            if (response_msg.data &&
                                ...
                                console.log("Current UCP id in approve: " + currentUCPId);
                            }
                            if (currentUCPId) {
                                let msg = DP_LeaveNotification.getMessageService().getSubscribeNotifMessage(currentUCPId, 'notification');
                                //console.log("[Approbation Notification_OBSERVER] Receiving incoming responses...")
                                let newSub = DP_LeaveNotification.subscribe(appname, msg).subscribe({
                                    ...
                                    error: (err) => {
                                        ...
                                        console.error('[Approbation Notification_OBSERVER]Something wrong occurred: ' + message);
                                        // Start the server's subscription
                                        let connectionError = {
                                            requestId: "Start_" + new export_1.Uuid().generateId(),
                                            eventType: handlers_manager_1.eventTypes.ConnectionError,
                                        ...
                                        //console.log("Approbation Notification 2 Connection Error!")
                                        this.errorSinkObservable.next(connectionError);
                                    },
                                    complete() {
                                        //console.log('Error');
                                        console.log('[Approbation Notification_OBSERVER]Done observable 2.');
                                    }
                                });
                        ...
                        error: (err) => {
                            console.error('[Approbation Notification_OBSERVER]Something wrong occurred: ' + err.message);
                            // Start the server's subscription
                            let connectionError = {
                                requestId: "Start_" + new export_1.Uuid().generateId(),
                                eventType: handlers_manager_1.eventTypes.ConnectionError,
                            ...
                            //console.log("Approbation Notification 1 Connection Error!")
                            this.errorSinkObservable.next(connectionError);
                        },
                        complete() {
                            //console.log('[Approbation Notification_OBSERVER]Done observable 1.');
                        },
                    });
    ...
    subscribeForCheckLeaveApprobation(inputObservable) {
        let newOutputObservable = new rxjs_1.Subject();
        inputObservable.subscribe({
            next: (process) => {
                if (process.eventType == handlers_manager_1.eventTypes.CheckLeaveApprobation) {
                    ...
                    if (operation_data > "" && operation_data.toLowerCase() == "post") {
                        console.log("Approbation Leave Detected..");
                        // Fixed handler_id issue
                        if (!process.requestId) {
                            this.add_handler(process.requestId, process.message, "", "Default");
                            // process.requestId = this.handlers.length - 1;
                        }
                        let message = {
        ...
        inputObservable.subscribe({
            next: (process) => {
                if (process.eventType == handlers_manager_1.eventTypes.UpdateCalendarLeave) {
                    ...
                    let webhooklink = this.n8n_url + "/webhook/leave-application";
                    if (httprequestparam > "") {
...
exports.approved_leave_application_notification_class = approved_leave_application_notification_class;
//# sourceMappingURL=approved_leave_application_notification_handler.js.map