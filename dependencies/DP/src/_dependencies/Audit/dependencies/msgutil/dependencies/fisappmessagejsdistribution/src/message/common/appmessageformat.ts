 ...
 */
import { MessageFormat, MessageParameter, DataFormat } from '../../types/appmessagetype';
 ...
 */
export function validate(messageFormat: MessageFormat): boolean {
    try {
        if (!messageFormat) {
...
export class AppMessageFormat {
     ...
     */
    public create(messageParameter: MessageParameter): MessageFormat {
        try {
            this.messageFormat = {} as MessageFormat;
            this.messageFormat.dataFormat = DataFormat.Json;     // Required. Default is JSON
 ...
 * @return {MessageFormat} - Message format.
 */
export function createMessageFormat(messageParameter: MessageParameter): MessageFormat {
    try {
