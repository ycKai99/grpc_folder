...
import {
    AppMessageTransformerCreator as MessageTransformer,
    transform
} from '../transform/appmessagetransformercreator';
...
function start() {
    ...
    try {
        if (argsCount > 0) {
            let data: Buffer = fs.readFileSync(args[0]);
...
function test(message: Message) {
    try {
        console.log(JSON.stringify(
            // MessageTransformer.transform(message), null, 2));
            transform(message), null, 2));
    }
    catch (e) {
        throw e;
    }
